  ...
  for (const city of cities) {
    try {
      const response = await fetch(
        `https://weather-analysis-hhcy.onrender.com/weather-detail?city=${city}`
      );
...
async function fetchDailySummary() {
  ...
  const response = await fetch(
    `https://weather-analysis-hhcy.onrender.com/weather-history/${city}?start_date=${formattedStartDate}&end_date=${formattedEndDate}`
  );
  const result = await response.json();
